...
- [ChoiceConfigData包装类](file://Assets/Scripts/Data/Configs/ChoiceConfig.cs)
- [ChoiceGroupConfigData包装类](file://Assets/Scripts/Data/Configs/ChoiceGroupConfig.cs)
...
**所做更改**
- 新增Choice配置系统的完整集成文档
- 更新ConfigManager和Cfg类以反映Choice配置表的添加
- 添加ChoiceConfig和ChoiceGroupConfig的数据结构说明
- 新增Choice配置文件的详细分析和使用指南
- 更新配置生成器以支持Choice配置的自动生成
- 添加Choice系统在游戏UI中的实际应用场景
...
**重大更新**：系统现已集成了Choice配置系统，为游戏中的选择分支、对话选项和剧情分支提供完整的配置支持。Choice系统包含独立的ChoiceConfig和ChoiceGroupConfig配置表，通过标准的配置访问模式与游戏逻辑解耦。
...
- **id**：选择组唯一标识符
- **title**：选择组标题
- **options**：选项数组，包含多个ChoiceConfig.OptionsItem
...
- [choice_group_config.json:1-298](file://Assets/Resources/Configs/choice_group_config.json#L1-L298)
...
ChoiceGroupConfig类处理选择组配置：

```csharp
public class ChoiceGroupConfig
{
    public class OptionsItem  // 嵌套类
    {
        public int id;
        public string text;
    ...
    public OptionsItem[] options; // 选项数组
}
```
...
- [ChoiceConfig.cs:10-26](file://Assets/Scripts/Data/Configs/ChoiceConfig.cs#L10-L26)
- [ChoiceGroupConfig.cs:10-34](file://Assets/Scripts/Data/Configs/ChoiceGroupConfig.cs#L10-L34)
- [ConfigManager.cs:161-164](file://Assets/Scripts/Core/ConfigManager.cs#L161-L164)
...
### ChoiceGroupConfig.OptionsItem详细字段说明

#### 选项基本信息
- **id**：整数类型，选项的唯一标识符
- **text**：字符串类型，选项显示文本
- **description**：字符串类型，选项详细描述

#### 选项效果
- **effectId**：整数类型，选项关联的效果ID
- **triggerBattle**：布尔类型，选择后是否触发战斗
- **goldReward**：整数类型，选择后的金币奖励
...
{
    foreach (var choice in Cfg.Choice.All)
    {
        if (string.IsNullOrEmpty(choice.text))
        {
            Debug.LogError($"选择项{id}缺少文本描述");
            return false;
        }
        if (choice.effectId > 0 && Cfg.PassiveEffect.Get(choice.effectId) == null)
        {
            Debug.LogWarning($"选择项{id}关联的效果不存在");
        }
    }
    return true;
}
...
**章节来源**
- [Cfg.cs:31-32](file://Assets/Scripts/Core/Cfg.cs#L31-L32)
- [ConfigManager.cs:161-164](file://Assets/Scripts/Core/ConfigManager.cs#L161-L164)
...
private GameObject CreateOptionItem(ChoiceGroupConfig.OptionsItem option, int optionIndex, Font font)
{
    ...
    Text nameText = itemObj.AddComponent<Text>();
...
lines.append("        public string description;")
...
lines.append("    {")
lines.append("        [Serializable]")
lines.append("        public class OptionsItem")
lines.append("        {")
lines.append("            public int id;")
lines.append("            public string text;")
lines.append("            public string description;")
...
lines.append("        public OptionsItem[] options;")
lines.append("    }")
```
...
- **选择组优化**：ChoiceGroupConfig.options数组支持快速遍历
...
**重大更新**：Choice配置系统的成功集成为游戏增加了强大的选择分支和对话系统支持。Choice系统具有以下优势：
- **模块化设计**：ChoiceConfig和ChoiceGroupConfig分离，支持独立和组合使用
...
- **Choice系统完整**：全新的Choice配置系统提供完整的交互体验

**重大变更影响**：
- **向后兼容性**：Choice配置文件的新增不影响现有配置的使用
- **平衡性调整**：Choice系统的效果需要重新平衡游戏性
- **配置迁移**：现有的选择逻辑需要迁移到新的Choice系统
- **生成器升级**：配置生成器已完全支持Choice配置的自动生成
...
- **创建ChoiceGroupConfig**：将相关的选项组合为ChoiceGroupConfig
...
**章节来源**
- [ChoiceConfig.cs:1-26](file://Assets/Scripts/Data/Configs/ChoiceConfig.cs#L1-L26)
- [ChoiceGroupConfig.cs:1-34](file://Assets/Scripts/Data/Configs/ChoiceGroupConfig.cs#L1-L34)
- [ChoiceWin.cs:1-222](file://Assets/Scripts/UI/ChoiceWin.cs#L1-L222)
- [config_gen.py:587-688](file://Tools/config_gen.py#L587-L688)