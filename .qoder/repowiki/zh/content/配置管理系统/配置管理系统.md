...
- [arcane_config.json:1-94](file://Assets/Resources/Configs/arcane_config.json#L1-L94)
...
```mermaid
flowchart LR
JSON["JSON配置文件"] --> Generator["配置生成器(config_gen.py)"]
Generator --> DataClass["配置数据类<br/>HeroConfigData.cs"]
Generator --> AccessClass["访问代理类<br/>Cfg.cs"]
DataClass --> Runtime["运行时配置表"]
AccessClass --> Runtime
```
...
### 配置文件编写规范
- **文件命名**：使用snake_case命名，如`hero_config.json`
...
### 扩展新配置类型的步骤
1. **创建JSON文件**：定义新的配置数据结构
2. **运行生成器**：执行config_gen.py生成配置类
...
- [SkillConfig.cs:1-43](file://Assets/Scripts/Data/Configs/SkillConfig.cs#L1-L43)