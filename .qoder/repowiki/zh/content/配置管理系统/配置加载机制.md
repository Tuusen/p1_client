...
## 更新摘要
**变更内容**
- 更新了配置加载顺序章节，反映ConfigManager中表初始化顺序的逻辑重组
- 新增了加载顺序与依赖关系的详细分析
- 更新了配置加载流程图以反映新的加载序列
- 增强了异常处理与错误恢复机制的说明
...
+passiveTable : ConfigTable~PassiveConfig~
+levelTable : ConfigTable~LevelConfig~
+arcaneTable : ConfigTable~ArcaneConfig~
...
LoadAll --> LoadEvent["加载事件配置<br/>LoadConfig<EventConfigData>()"]
LoadEvent --> InitEvent["初始化EventTable<br/>Init(items, id)"]
InitEvent --> LoadBulletEvent["加载子弹事件配置<br/>LoadConfig<BulletEventConfigData>()"]
LoadBulletEvent --> InitBulletEvent["初始化BulletEventTable<br/>Init(items, id)"]
InitBulletEvent --> LoadBuff["加载Buff配置<br/>LoadConfig<BuffConfigData>()"]
LoadBuff --> InitBuff["初始化BuffTable<br/>Init(items, id)"]
InitBuff --> LoadPassive["加载被动配置<br/>LoadConfig<PassiveConfigData>()"]
LoadPassive --> InitPassive["初始化PassiveTable<br/>Init(items, id)"]
InitPassive --> LoadLevel["加载关卡配置<br/>LoadConfig<LevelConfigData>()"]
LoadLevel --> InitLevel["初始化LevelTable<br/>Init(items, id)"]
InitLevel --> LoadArcane["加载奥术配置<br/>LoadConfig<ArcaneConfigData>()"]
LoadArcane --> InitArcane["初始化ArcaneTable<br/>Init(items, id)"]
InitArcane --> LoadDialogue["加载对话配置<br/>LoadConfig<DialogueConfigData>()"]
LoadDialogue --> InitDialogue["初始化DialogueTable<br/>Init(items, id)"]
InitDialogue --> LoadAttribute["加载属性配置<br/>LoadConfig<AttributeConfigData>()"]
LoadAttribute --> InitAttribute["初始化AttributeTable<br/>Init(items, id)"]
InitAttribute --> LoadRole["加载角色配置<br/>LoadConfig<RoleConfigData>()"]
LoadRole --> InitRole["初始化RoleTable<br/>Init(items, id)"]
InitRole --> LoadMonster["加载怪物配置<br/>LoadConfig<MonsterConfigData>()"]
LoadMonster --> InitMonster["初始化MonsterTable<br/>Init(items, id)"]
InitMonster --> LoadSkillPool["加载技能池配置<br/>LoadConfig<SkillPoolConfigData>()"]
LoadSkillPool --> InitSkillPool["初始化SkillPoolTable<br/>Init(items, id)"]
InitSkillPool --> LoadSkill["加载技能配置<br/>LoadConfig<SkillConfigData>()"]
LoadSkill --> InitSkill["初始化SkillTable<br/>Init(items, id)"]
...
**更新** 配置加载顺序已进行逻辑重组，从原来的"基础配置(ArcaneConfig, AttributeConfig)"到"复杂系统(EventConfig, BulletEventConfig, BuffConfig, PassiveConfig)"的重新排列：

1. **复杂系统配置**：event_config.json、bullet_event_config.json、buff_config.json、passive_config.json、level_config.json
2. **基础配置**：arcane_config.json、attribute_config.json、role_config.json
3. **角色配置**：hero_config.json、monster_config.json
4. **技能配置**：skill_pool_config.json、skill_config.json
5. **剧情配置**：story_collection_config.json、story_node_config.json、choice_group_config.json、choice_config.json
6. **效果配置**：passive_effect_config.json、event_shop_config.json、condition_config.json、event_effect_config.json、bullet_style_config.json、dialogue_config.json

**重新排列的原因**：
- 复杂系统配置需要在基础配置之前加载，因为它们可能依赖于基础配置中的某些设置
- 关卡配置(level_config)需要在角色配置之前加载，以确保关卡数据能够正确引用角色配置
- 技能配置(skill_pool_config, skill_config)需要在角色配置之后加载，以确保技能数据能够正确关联到角色
...
**章节来源**
- [HeroConfig.cs:10-38](file://Assets/Scripts/Data/Configs/HeroConfig.cs#L10-L38)
- [arcane_config.json:1-76](file://Assets/Resources/Configs/arcane_config.json#L1-L76)
- [attribute_config.json:1-319](file://Assets/Resources/Configs/attribute_config.json#L1-L319)
- [event_config.json:1-800](file://Assets/Resources/Configs/event_config.json#L1-L800)
- [buff_config.json:1-800](file://Assets/Resources/Configs/buff_config.json#L1-L800)
- [bullet_event_config.json:1-200](file://Assets/Resources/Configs/bullet_event_config.json#L1-L200)
