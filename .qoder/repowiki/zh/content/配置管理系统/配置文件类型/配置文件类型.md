...
## 更新摘要
**所做更改**
- 新增了Choice配置系统，包含choice_config.json和choice_group_config.json两个新文件类型
- 更新了ConfigManager配置加载逻辑，新增了choiceTable和choiceGroupTable
- 完善了故事集系统的决策机制，支持动态选择和分支剧情
...
**更新** 配置系统已升级至新架构，新增了Choice配置系统，包含choice_config.json和choice_group_config.json等新文件类型，支持动态决策机制和分支剧情。ConfigManager已更新配置加载逻辑，体现了更清晰的配置分离和更好的扩展性。
...
- [ChoiceGroupConfig.cs:10-33](file://Assets/Scripts/Data/Configs/ChoiceGroupConfig.cs#L10-L33)
- [ChoiceConfig.cs:10-21](file://Assets/Scripts/Data/Configs/ChoiceConfig.cs#L10-L21)
...
- **字段**
  ...
  - ChoiceGroupConfig.options: OptionsItem数组，包含选项详情
  - OptionsItem.id: 整数，选项ID
  - OptionsItem.text: 字符串，选项文本
  - OptionsItem.description: 字符串，描述
  - OptionsItem.effectId: 整数，效果ID
  - OptionsItem.triggerBattle: 布尔，是否触发战斗
  - OptionsItem.goldReward: 整数，金币奖励
- **用途**：定义可选项及其效果，替代旧的选择选项配置
- **更新**：采用items数组结构，每个选择组包含多个选项
...
- **Choice系统**
  ...
  - choice_group_config 的 options 通过 effectId 引用效果配置
...
- **更新**：验证 choice_group_config.json 的 options 数组结构；检查选项ID的编号规则是否符合要求
...
## 结论
本文件系统化梳理了 GeometryTD 的配置体系，涵盖了基础配置、事件系统、被动系统、故事集系统等所有配置文件类型。随着配置系统的升级，新增了Choice配置系统，包含choice_config.json和choice_group_config.json等新文件类型，支持动态决策机制和分支剧情。ConfigManager已更新配置加载逻辑，体现了更清晰的配置分离和更好的扩展性。明确了各类配置的数据结构、字段语义、引用关系与运行时加载机制。新架构采用 ConfigTable 和 ConfigMeta 模式，提供了更好的性能和可维护性。为策划与开发者提供了完整的配置维护指导。
