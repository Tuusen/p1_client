...
## 更新摘要
**所做更改**
- 更新了工程wiki生成系统的建立情况说明
- 补充了新增的配置文件类型（事件系统、被动系统、故事集系统等）
- 完善了配置文件的分类说明和依赖关系分析
...
**更新** 工程wiki生成系统已建立，删除过时的详细解释文档，现有配置文件类型文档已足够覆盖所有配置文件说明。
...
- **故事集系统组件**
  ...
  - 选择配置 choice_option_config：定义可选项及其效果
...
- [GameConfigs.cs:122-152](file://Assets/Scripts/Data/GameConfigs.cs#L122-L152)
...
- [GameConfigs.cs:245-271](file://Assets/Scripts/Data/GameConfigs.cs#L245-L271)
...
#### 选择配置 choice_option_config
- **字段**
  - id/title/options: 选择组定义
  - options: 选项数组，包含选项ID、文本、描述、效果ID、是否触发战斗、金币奖励等
- **用途**：定义可选项及其效果
...
### 故事集系统依赖关系
- **故事节点**
  - story_node_config 引用 level_config、dialogue_config、choice_option_config、event_shop_config
  - 定义了故事流程的节点类型与连接关系
- **条件系统**
...
SNC["story_node_config"] --> LC
SNC --> DC
SNC --> COC
ESC["event_shop_config"] --> SNC
```
...
### 故事集系统问题
- **现象**：故事流程异常、对话显示错误
- **排查**：检查 story_node_config 的节点连接关系；确认 dialogue_config/choice_option_config 的ID引用正确
...
## 结论
本文件系统化梳理了 GeometryTD 的配置体系，涵盖了基础配置、事件系统、被动系统、故事集系统等所有配置文件类型。明确了各类配置的数据结构、字段语义、引用关系与运行时加载机制。随着工程wiki生成系统的建立，现有配置文件类型文档已足够覆盖所有配置文件说明，为策划与开发者提供了完整的配置维护指导。
