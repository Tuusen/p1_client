...
## 更新摘要
**所做更改**
- 新增了三个配置文件类型：bullet_style_config.json、choice_group_config.json、global_config.json
- 移除了两个旧的配置文件类型：bullet_config.json、choice_option_config.json
- 更新了配置系统架构，采用新的数据结构和加载机制
- 完善了配置文件的分类说明和依赖关系分析
- 更新了配置文件的版本管理和兼容性处理机制
...
**更新** 配置系统已升级至新架构，新增了bullet_style_config.json、choice_group_config.json和global_config.json等新文件类型，同时移除了旧的bullet_config.json和choice_option_config.json，体现了更清晰的配置分离和更好的扩展性。
...
- **预加载与缓存**
  ...
  - 新增了 bulletStyleTable 和 choiceGroupTable 的索引机制
- **查找索引**
...
- **更新**：检查新文件类型如 bullet_style_config.json、choice_group_config.json、global_config.json 的格式
...
## 结论
本文件系统化梳理了 GeometryTD 的配置体系，涵盖了基础配置、事件系统、被动系统、故事集系统等所有配置文件类型。随着配置系统的升级，新增了 bullet_style_config.json、choice_group_config.json 和 global_config.json 等新文件类型，移除了旧的 bullet_config.json 和 choice_option_config.json，体现了更清晰的配置分离和更好的扩展性。明确了各类配置的数据结构、字段语义、引用关系与运行时加载机制。新架构采用 ConfigTable 和 ConfigMeta 模式，提供了更好的性能和可维护性。为策划与开发者提供了完整的配置维护指导。
