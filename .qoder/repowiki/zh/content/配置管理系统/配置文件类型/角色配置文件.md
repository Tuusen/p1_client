...
**更新** 新增动画控制器标准化映射功能，通过角色配置中的animator字段统一管理所有角色类型的动画控制器资源。
...
RC["role_config.json<br/>新增animator字段"]
...
- 角色类型配置（role_config.json）
  - 字段要点：id、name、animator、prefabPath、portraitPath
  - 说明：将角色id映射到动画控制器路径、预制体路径与头像路径，供运行时实例化与UI展示
...
- 动画控制器通过role_config中的animator字段统一管理
...
- 数据结构
  - 角色列表：roles[]
  - 每个角色条目包含：id、name、animator、prefabPath、portraitPath
- 新增字段详解
  - animator：动画控制器资源路径，标准化所有角色类型的动画映射
  - prefabPath：预制体资源路径
  - portraitPath：头像资源路径
- 作用
  - 将角色id映射到动画控制器、预制体与头像资源路径，供运行时加载与UI展示
  - animator字段统一管理各角色类型的动画资源，确保动画系统的一致性
- 设计要点
  - animator路径需与实际资源一致，避免运行时警告
  - 不同角色类型（Hero/Monster/Boss/Summon）对应不同的动画控制器目录
  - 标准化的动画映射简化了运行时的动画控制器查找与绑定过程
...
- 配置文件间依赖
  ...
  - role_config.json新增animator字段依赖各角色类型的AnimatorController资源
...
- 潜在耦合点
  ...
  - animator字段的路径必须与实际资源路径完全匹配

```mermaid
graph LR
RC["role_config.json<br/>含animator字段"] --> CM["ConfigManager.cs"]
...
- 动画系统
  - animator字段的标准化映射减少了运行时动画控制器的查找开销
  - 统一的动画资源管理提高了内存使用效率
...
- 动画控制器加载失败
  - 现象：日志出现"无法加载AnimatorController"或动画不播放
  - 排查：核对role_config.json中的animator字段路径是否正确；确认对应的.controller文件存在且命名一致
- 属性异常
  - 现象：属性上下限未生效或百分比换算错误
...
## 结论
角色配置体系通过清晰的JSON结构与严谨的数据模型，实现了从基础属性到技能事件的完整闭环。新增的animator字段标准化了所有角色类型的动画控制器映射，统一了动画资源管理，提升了系统的可维护性和运行时性能。合理利用属性元数据、技能池与Buff系统，可快速构建多样化角色并维持数值平衡。建议在策划层面建立配置规范与校验流程，在开发层面强化索引与缓存策略，以确保配置的可维护性与运行时性能。
...
- 角色类型（role_config.json）
  - id、name、animator、prefabPath、portraitPath
- 属性元数据（attribute_config.json）
...
- 性能
  ...
  - 预加载常用资源，避免首次使用抖动
  - 合理拆分配置文件，按需加载与缓存
  - 利用标准化的animator字段统一管理动画资源，提升加载效率
- 动画系统优化
  - 统一使用animator字段管理动画控制器，避免分散的资源引用
  - 确保各角色类型的动画控制器命名规范一致
  - 合理组织动画资源目录结构，便于维护和扩展

### 动画控制器配置示例
- 英雄动画控制器：Characters/Hero/sword_man.controller
- 怪物动画控制器：Characters/Monster/goblin.controller
- Boss动画控制器：Characters/Boss/skull_warrior.controller
- 召唤物动画控制器：Characters/Summon/green_slime.controller

这些控制器文件定义了各角色类型的动画状态机、参数和过渡条件，通过role_config.json中的animator字段进行统一管理。