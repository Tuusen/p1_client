...
**图表来源**
- [BulletController.cs:1-345](file://Assets/Scripts/Battle/BulletController.cs#L1-L345)
...
**章节来源**
- [BulletController.cs:32-345](file://Assets/Scripts/Battle/BulletController.cs#L32-L345)
...
PierceMode --> |否| Move["按方向推进"]
PierceCheck --> Hit{"是否命中?"}
...
## 结论
弹幕系统以 BulletController 为核心，结合 BulletEventExecutor 的事件装配、ConfigManager 的配置管理、DamageCalculator 的伤害公式与 EventExecutor 的效果执行，形成清晰的职责分层。系统支持多种弹幕效果（追踪、爆炸、弹射、穿透、散射、连射、齐射）并通过事件附加实现丰富的命中后效果。通过对象池、碰撞优化与生命周期控制可显著提升性能。扩展方面，新增事件类型与弹幕样式均具备明确的接入点与配置入口。
...
- 关键算法实现路径参考
  ...
  - 碰撞检测（穿透）：[BulletController.cs:136-151](file://Assets/Scripts/Battle/BulletController.cs#L136-L151)
