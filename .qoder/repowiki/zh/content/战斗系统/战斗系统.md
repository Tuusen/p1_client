...
**更新** 本次更新重点介绍了 EventEffectManager 的视觉特效管理系统和 BulletController 的 homing 追踪机制改进。
...
BattleManager --> EventEffectManager : "管理/调用"
...
BS --> DC
EEM --> EEConfig["event_effect_config.json"]
```
...
## 结论
该战斗系统通过 BattleManager 统一调度，结合角色自治与纯函数计算，实现了稳定、可扩展的回合制与实时战斗融合体验。属性、Buff、被动与伤害计算模块清晰解耦，便于后续扩展新敌人、Boss 与特殊场景。
