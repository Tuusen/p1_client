...
- 控制器层：HeroController、MonsterController、BossController
...
## 核心组件
- 战斗管理器（BattleManager）：负责战斗生命周期、敌人列表管理、范围伤害、生成接口、结果判定、UI联动与事件链处理。
...
- [HeroController.cs:178-206](file://Assets/Scripts/Battle/HeroController.cs#L178-L206)
- [HeroController.cs:207-281](file://Assets/Scripts/Battle/HeroController.cs#L207-L281)
- [HeroController.cs:284-399](file://Assets/Scripts/Battle/HeroController.cs#L284-L399)
...
## 结论
BattleManager通过清晰的职责划分与模块化设计，实现了回合制战斗的核心流程：初始化、生成、战斗、事件与结算。其公共接口覆盖了敌人查询、范围伤害与生成三大类能力，并通过SkillManager与ArcaneManager提供了可扩展的技能与机制。性能方面，通过每帧清理、内存回收与计时器优化确保了稳定运行。建议在新增战斗场景时，遵循现有接口与事件链模式，保持低耦合与高内聚。
