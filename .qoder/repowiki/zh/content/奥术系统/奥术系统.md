# 奥术系统

<cite>
**本文档引用的文件**
- [ArcaneManager.cs](file://Assets/Scripts/Battle/ArcaneManager.cs)
- [ArcaneSlotUI.cs](file://Assets/Scripts/UI/Components/ArcaneSlotUI.cs)
- [ArcaneBarUI.cs](file://Assets/Scripts/UI/ArcaneBarUI.cs)
- [ArcaneActiveIconUI.cs](file://Assets/Scripts/UI/ArcaneActiveIconUI.cs)
- [RuneBarUI.cs](file://Assets/Scripts/UI/RuneBarUI.cs)
- [ArcaneConfig.cs](file://Assets/Scripts/Data/Configs/ArcaneConfig.cs)
- [ArcaneSelectWin.cs](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs)
- [BaseWin.cs](file://Assets/Scripts/UI/Windows/BaseWin.cs)
- [DragVisualManager.cs](file://Assets/Scripts/UI/Components/DragVisualManager.cs)
- [DragHintUI.cs](file://Assets/Scripts/UI/Components/DragHintUI.cs)
- [ConfigManager.cs](file://Assets/Scripts/Core/ConfigManager.cs)
- [GameConfigs.cs](file://Assets/Scripts/Data/GameConfigs.cs)
- [arcane_config.json](file://Assets/Resources/Configs/arcane_config.json)
- [bullet_event_config.json](file://Assets/Resources/Configs/bullet_event_config.json)
- [EventExecutor.cs](file://Assets/Scripts/Battle/EventExecutor.cs)
- [BuffSystem.cs](file://Assets/Scripts/Battle/BuffSystem.cs)
- [BattleManager.cs](file://Assets/Scripts/Battle/BattleManager.cs)
- [ArcaneSelectWin.prefab](file://Assets/Resources/UI/Windows/ArcaneSelectWin.prefab)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件为 GeometryTD 的奥术系统技术文档，围绕符文槽位管理、能量收集与冷却系统、符文配置与效果、组合机制、激活触发与执行流程、平衡性设计以及扩展指南进行系统化说明。文档面向开发者与策划，既提供代码级实现细节，也给出可视化图示与实践建议。

**更新** 本次更新反映了 ArcaneSelectWin.cs 的完全重构，采用新的命名约定和自动绑定系统，显著提升了代码的可维护性和开发效率。

## 项目结构
- 配置层：arcane_config.json 定义符文属性与效果；bullet_event_config.json 定义子弹事件；GameConfigs 提供数据模型与枚举。
- 管理层：ArcaneManager 负责符文槽位、能量、冷却与激活逻辑；ConfigManager 负责加载与索引配置。
- 游戏层：BattleManager 协调战斗流程，调用 ArcaneManager 执行符文；EventExecutor 执行事件链。
- UI层：ArcaneBarUI/ArcaneSlotUI/RuneBarUI/ArcaneActiveIconUI 负责显示符文槽、符能、冷却与激活图标；ArcaneSelectWin 负责符文选择界面，采用新的自动绑定系统。

```mermaid
graph TB
subgraph "配置层"
CFG["ConfigManager<br/>加载与索引"]
ARC_CFG["arcane_config.json<br/>符文配置"]
BUL_CFG["bullet_event_config.json<br/>子弹事件配置"]
GC["GameConfigs<br/>数据模型/枚举"]
AC["ArcaneConfig<br/>dragHint属性"]
end
subgraph "管理层"
AM["ArcaneManager<br/>符文/能量/冷却/激活"]
BM["BattleManager<br/>战斗协调"]
BE["EventExecutor<br/>事件执行"]
BS["BuffSystem<br/>增益/减益/特殊效果"]
end
subgraph "UI层"
ABAR["ArcaneBarUI"]
ASLOT["ArcaneSlotUI<br/>拖拽提示"]
RBAR["RuneBarUI"]
AICON["ArcaneActiveIconUI"]
ASEL["ArcaneSelectWin<br/>自动绑定系统"]
BASEWIN["BaseWin<br/>自动UI绑定"]
DV["DragVisualManager<br/>拖拽视觉管理"]
DH["DragHintUI<br/>拖拽提示UI"]
end
ARC_CFG --> CFG
BUL_CFG --> CFG
GC --> CFG
AC --> AM
CFG --> AM
BM --> AM
AM --> BE
AM --> BS
AM --> ABAR
AM --> RBAR
AM --> AICON
ABAR --> ASLOT
ASEL --> BASEWIN
ASEL --> DV
BASEWIN --> DV
DV --> DH
```

**图表来源**
- [ConfigManager.cs:77-122](file://Assets/Scripts/Core/ConfigManager.cs#L77-L122)
- [ArcaneManager.cs:23-56](file://Assets/Scripts/Battle/ArcaneManager.cs#L23-L56)
- [ArcaneBarUI.cs:5-28](file://Assets/Scripts/UI/ArcaneBarUI.cs#L5-L28)
- [RuneBarUI.cs:6-33](file://Assets/Scripts/UI/RuneBarUI.cs#L6-L33)
- [ArcaneActiveIconUI.cs:7-25](file://Assets/Scripts/UI/ArcaneActiveIconUI.cs#L7-L25)
- [ArcaneConfig.cs:27](file://Assets/Scripts/Data/Configs/ArcaneConfig.cs#L27)
- [ArcaneSelectWin.cs:14](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L14)
- [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)
- [DragVisualManager.cs:21](file://Assets/Scripts/UI/Components/DragVisualManager.cs#L21)
- [DragHintUI.cs:10](file://Assets/Scripts/UI/Components/DragHintUI.cs#L10)
- [EventExecutor.cs:13-20](file://Assets/Scripts/Battle/EventExecutor.cs#L13-L20)
- [BuffSystem.cs:30-84](file://Assets/Scripts/Battle/BuffSystem.cs#L30-L84)
- [BattleManager.cs:240-274](file://Assets/Scripts/Battle/BattleManager.cs#L240-L274)

**章节来源**
- [ConfigManager.cs:77-122](file://Assets/Scripts/Core/ConfigManager.cs#L77-L122)
- [ArcaneManager.cs:23-56](file://Assets/Scripts/Battle/ArcaneManager.cs#L23-L56)
- [ArcaneBarUI.cs:5-28](file://Assets/Scripts/UI/ArcaneBarUI.cs#L5-L28)
- [RuneBarUI.cs:6-33](file://Assets/Scripts/UI/RuneBarUI.cs#L6-L33)
- [ArcaneActiveIconUI.cs:7-25](file://Assets/Scripts/UI/ArcaneActiveIconUI.cs#L7-L25)
- [ArcaneConfig.cs:27](file://Assets/Scripts/Data/Configs/ArcaneConfig.cs#L27)
- [ArcaneSelectWin.cs:14](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L14)
- [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)
- [DragVisualManager.cs:21](file://Assets/Scripts/UI/Components/DragVisualManager.cs#L21)
- [DragHintUI.cs:10](file://Assets/Scripts/UI/Components/DragHintUI.cs#L10)
- [EventExecutor.cs:13-20](file://Assets/Scripts/Battle/EventExecutor.cs#L13-L20)
- [BuffSystem.cs:30-84](file://Assets/Scripts/Battle/BuffSystem.cs#L30-L84)
- [BattleManager.cs:240-274](file://Assets/Scripts/Battle/BattleManager.cs#L240-L274)

## 核心组件
- 符文槽位与状态
  - ArcaneSlotState：记录每个槽位的符文ID、名称、剩余冷却与最大冷却。
  - ArcaneManager.slots：固定长度数组，对应UI栏位数量。
- 能量与符能
  - energy：每种元素能量（0-9），满10进1符能。
  - runes：每种元素符能（可消耗用于施放）。
- 激活与周期
  - ActiveArcane：记录激活符文的位置、周期、计时与配置。
  - 每帧更新冷却与激活周期，周期到触发伤害/事件。
- 触发与效果
  - CanCast：检查冷却与符能消耗（含Buff修饰）。
  - TryCastArcane：消耗符能、进入冷却、创建ActiveArcane。
  - TickArcane：按配置计算伤害、范围命中、执行事件链。
- **新增** 自动绑定系统
  - BaseWin.AutoBindUIComponents：自动根据命名规则绑定UI组件，支持 txt_、btn_、toggle_、node_ 等前缀。
  - ArcaneSelectWin：继承 BaseWin，利用自动绑定系统简化UI组件初始化。
- **新增** 拖拽提示系统
  - ArcaneConfig.dragHint：符文配置中的拖拽提示文本。
  - DragVisualManager：管理拖拽过程中的视觉效果和提示。
  - DragHintUI：显示拖拽提示的UI组件。

**章节来源**
- [ArcaneManager.cs:6-28](file://Assets/Scripts/Battle/ArcaneManager.cs#L6-L28)
- [ArcaneManager.cs:119-165](file://Assets/Scripts/Battle/ArcaneManager.cs#L119-L165)
- [ArcaneManager.cs:198-256](file://Assets/Scripts/Battle/ArcaneManager.cs#L198-L256)
- [ArcaneConfig.cs:27](file://Assets/Scripts/Data/Configs/ArcaneConfig.cs#L27)
- [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)
- [ArcaneSelectWin.cs:18-26](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L18-L26)
- [DragVisualManager.cs:29](file://Assets/Scripts/UI/Components/DragVisualManager.cs#L29)
- [DragHintUI.cs:14](file://Assets/Scripts/UI/Components/DragHintUI.cs#L14)

## 架构总览
下图展示从用户交互到符文激活、伤害计算与事件执行的完整流程，包括新增的自动绑定系统和拖拽提示系统。

```mermaid
sequenceDiagram
participant U as "用户"
participant UI as "ArcaneSlotUI"
participant DV as "DragVisualManager"
participant AM as "ArcaneManager"
participant BM as "BattleManager"
participant E as "EventExecutor"
U->>UI : "拖拽符文槽位"
UI->>DV : "BeginDrag(dragHint)"
DV->>UI : "显示拖拽提示"
UI->>AM : "TryCastArcane(槽位, 世界坐标)"
AM->>AM : "CanCast检查冷却/符能"
AM->>AM : "消耗符能/进入冷却"
AM->>BM : "创建ActiveArcane"
AM->>DV : "EndDrag()"
DV-->>UI : "隐藏拖拽提示"
AM->>AM : "Update每帧更新冷却/周期"
AM->>E : "TickArcane触发事件"
E-->>BM : "执行伤害/效果"
```

**图表来源**
- [ArcaneSlotUI.cs:103-172](file://Assets/Scripts/UI/Components/ArcaneSlotUI.cs#L103-L172)
- [DragVisualManager.cs:29](file://Assets/Scripts/UI/Components/DragVisualManager.cs#L29)
- [ArcaneManager.cs:135-165](file://Assets/Scripts/Battle/ArcaneManager.cs#L135-L165)
- [ArcaneManager.cs:167-196](file://Assets/Scripts/Battle/ArcaneManager.cs#L167-L196)
- [ArcaneManager.cs:198-256](file://Assets/Scripts/Battle/ArcaneManager.cs#L198-L256)
- [EventExecutor.cs:13-20](file://Assets/Scripts/Battle/EventExecutor.cs#L13-L20)

## 详细组件分析

### 符文槽位与UI交互
- ArcaneSlotUI
  - 拖拽开始/拖拽中/拖拽结束：创建拖影、范围圈、屏幕坐标到世界坐标的转换。
  - **新增** 拖拽提示：从配置中获取 `dragHint` 文本，通过 `DragVisualManager` 显示拖拽提示。
  - 结束拖拽且不在栏位内：调用 ArcaneManager.TryCastArcane。
  - UpdateSlot：根据冷却、符能与可用性更新UI（透明度、颜色、冷却覆盖）。
- ArcaneBarUI
  - 将 ArcaneManager 的槽位状态同步到各 ArcaneSlotUI。
- ArcaneActiveIconUI
  - 监听 OnArcanePlaced，动态生成激活图标并显示剩余周期。

```mermaid
flowchart TD
Start(["拖拽开始"]) --> GetHint["获取dragHint配置"]
GetHint --> ShowHint["显示拖拽提示"]
ShowHint --> Check["CanCast检查"]
Check --> |否| Cancel["取消拖拽/提示不可用"]
Check --> |是| Ghost["创建拖影/范围圈"]
Ghost --> Drag["跟随鼠标移动"]
Drag --> Drop["拖拽结束"]
Drop --> HideHint["隐藏拖拽提示"]
HideHint --> Pos["屏幕坐标转世界坐标"]
Pos --> Cast["ArcaneManager.TryCastArcane"]
Cast --> Done(["施放完成"])
```

**图表来源**
- [ArcaneSlotUI.cs:103-172](file://Assets/Scripts/UI/Components/ArcaneSlotUI.cs#L103-L172)
- [ArcaneSlotUI.cs:117-122](file://Assets/Scripts/UI/Components/ArcaneSlotUI.cs#L117-L122)
- [DragVisualManager.cs:29](file://Assets/Scripts/UI/Components/DragVisualManager.cs#L29)
- [DragHintUI.cs:14](file://Assets/Scripts/UI/Components/DragHintUI.cs#L14)

**章节来源**
- [ArcaneSlotUI.cs:103-172](file://Assets/Scripts/UI/Components/ArcaneSlotUI.cs#L103-L172)
- [ArcaneSlotUI.cs:49-97](file://Assets/Scripts/UI/Components/ArcaneSlotUI.cs#L49-L97)
- [ArcaneBarUI.cs:18-27](file://Assets/Scripts/UI/ArcaneBarUI.cs#L18-L27)
- [ArcaneActiveIconUI.cs:27-87](file://Assets/Scripts/UI/ArcaneActiveIconUI.cs#L27-L87)

### 符文选择界面重构与自动绑定系统
- ArcaneSelectWin（完全重构）
  - **新增** 继承 BaseWin：采用统一的窗口基类，获得自动绑定系统支持。
  - **新增** 自动UI绑定：通过 BaseWin.AutoBindUIComponents 自动绑定字段，无需[SerializeField]声明。
  - **新增** 命名约定：支持 txt_count、btn_enter、btn_close、node_list 等命名规则。
  - **新增** 选择状态显示：通过 txt_count 组件显示已选择的符文数量。
  - **新增** 选择限制：`MaxCount = 4` 限制最多选择4个符文。
  - **新增** 选择集合：`selectedIds` HashSet 跟踪已选择的符文ID。
  - **新增** 视觉反馈：`UpdateVisuals()` 方法根据选择状态更新背景颜色。
  - **新增** 确认功能：点击确认按钮保存选择并排序。
- BaseWin（自动绑定系统）
  - **新增** AutoBindUIComponents：自动查找匹配的子对象并绑定到对应字段。
  - **新增** 命名规则支持：完全匹配、去除前缀匹配、后缀匹配三种模式。
  - **新增** 事件自动绑定：Button 和 Toggle 组件自动绑定 onClick 和 onValueChanged 事件。
  - **新增** 类型安全绑定：根据字段类型进行相应的组件绑定和事件注册。

```mermaid
flowchart TD
Init(["窗口初始化"]) --> AutoBind["AutoBindUIComponents自动绑定"]
AutoBind --> FieldLoop{"遍历字段"}
FieldLoop --> Match["查找匹配子对象"]
Match --> Bind["根据类型绑定组件"]
Bind --> EventBind["绑定事件处理器"]
EventBind --> FieldLoop
FieldLoop --> |完成| Start["start()方法"]
Start --> Load["加载已装备符文"]
Load --> Create["创建符文项"]
Create --> Loop{"循环符文"}
Loop --> Click["点击符文"]
Click --> Check{"检查选择状态"}
Check --> |已选择| Remove["移除选择"]
Check --> |未选择且<4| Add["添加到选择"]
Remove --> Update["更新视觉状态"]
Add --> Update
Update --> Loop
Loop --> |完成| Confirm["点击确认"]
Confirm --> Save["保存选择并排序"]
Save --> Close(["关闭窗口"])
```

**图表来源**
- [ArcaneSelectWin.cs:28-84](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L28-L84)
- [ArcaneSelectWin.cs:137-145](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L137-L145)
- [ArcaneSelectWin.cs:147-158](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L147-L158)
- [ArcaneSelectWin.cs:160-168](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L160-L168)
- [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)

**章节来源**
- [ArcaneSelectWin.cs:14](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L14)
- [ArcaneSelectWin.cs:18-26](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L18-L26)
- [ArcaneSelectWin.cs:28-84](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L28-L84)
- [ArcaneSelectWin.cs:137-145](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L137-L145)
- [ArcaneSelectWin.cs:147-158](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L147-L158)
- [ArcaneSelectWin.cs:160-168](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L160-L168)
- [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)

### 能量收集与冷却系统
- 能量收集
  - EventExecutor.HandleGainEnergy：根据事件参数为指定元素类型增加能量（0-9）。
  - ArcaneManager.AddEnergy：累加能量，满10进1符能，并触发 OnRunesChanged。
- 冷却系统
  - ArcaneManager.Update：逐槽更新冷却，冷却归零触发 OnArcaneCooldownDone。
  - ArcaneSlotUI：显示冷却覆盖与剩余时间文本。
- 符能消耗与修饰
  - ArcaneManager.GetModifiedCost：结合 BuffSystem.GetArcaneCostModifier 对符能消耗进行加成/减免。
  - ArcaneManager.TryCastArcane：按修饰后的消耗扣减符能。

```mermaid
flowchart TD
Gain["事件: 获得能量"] --> AE["AddEnergy(元素类型, 数量)"]
AE --> Check10{"能量>=10?"}
Check10 --> |是| Rune["能量-=10<br/>符能+=1"]
Check10 --> |否| Wait["等待更多能量"]
Rune --> Notify["OnRunesChanged通知UI"]
Wait --> Notify
```

**图表来源**
- [EventExecutor.cs:124-143](file://Assets/Scripts/Battle/EventExecutor.cs#L124-L143)
- [ArcaneManager.cs:81-106](file://Assets/Scripts/Battle/ArcaneManager.cs#L81-L106)
- [BuffSystem.cs:285-303](file://Assets/Scripts/Battle/BuffSystem.cs#L285-L303)

**章节来源**
- [EventExecutor.cs:124-143](file://Assets/Scripts/Battle/EventExecutor.cs#L124-L143)
- [ArcaneManager.cs:81-106](file://Assets/Scripts/Battle/ArcaneManager.cs#L81-L106)
- [ArcaneManager.cs:167-196](file://Assets/Scripts/Battle/ArcaneManager.cs#L167-L196)
- [ArcaneSlotUI.cs:63-78](file://Assets/Scripts/UI/Components/ArcaneSlotUI.cs#L63-L78)
- [RuneBarUI.cs:35-54](file://Assets/Scripts/UI/RuneBarUI.cs#L35-L54)
- [BuffSystem.cs:285-303](file://Assets/Scripts/Battle/BuffSystem.cs#L285-L303)

### 符文配置与效果类型
- 配置来源
  - ConfigManager.LoadAllConfigs：加载 arcane_config.json 并建立 ArcaneConfigs 与索引。
  - ArcaneConfig：包含 id、name、des、icon、dmg、dmgType、radius、tickInterval、cd、runeCost、runeType、events/enemyEvents/bulletEvents、**dragHint**。
- 效果类型
  - 主动效果：由 ArcaneManager.TickArcane 执行，按配置计算伤害与范围命中。
  - 被动效果：通过 events/enemyEvents/bulletEvents 调用 EventExecutor 执行。
  - 全屏AoE：当 radius < 0 时，BattleManager.DealFullScreenAoe 对全体敌人造成伤害并附加事件。
- 元素类型
  - dmgType：0=无属性, 1=火, 2=冰, 3=电, 4=风；用于伤害计算与UI视觉区分。
- **新增** 拖拽提示
  - dragHint：每个符文的拖拽提示文本，用于指导玩家如何使用该符文。

```mermaid
classDiagram
class ArcaneConfig {
+int id
+string name
+string des
+string icon
+int dmg
+int dmgType
+float radius
+float tickInterval
+float cd
+int runeCost
+int runeType
+int[] events
+int[] enemyEvents
+int[] bulletEvents
+string dragHint
}
class ConfigManager {
+ArcaneConfig[] ArcaneConfigs
+GetArcaneConfig(id) ArcaneConfig
}
class ArcaneManager {
+Init(arcaneSlotIds, hero, bm)
+CanCast(slotIndex) bool
+TryCastArcane(slotIndex, pos) bool
+GetActiveArcanes() ActiveArcane[]
}
ConfigManager --> ArcaneConfig : "提供"
ArcaneManager --> ArcaneConfig : "读取"
```

**图表来源**
- [GameConfigs.cs:429-446](file://Assets/Scripts/Data/GameConfigs.cs#L429-L446)
- [ConfigManager.cs:258-272](file://Assets/Scripts/Core/ConfigManager.cs#L258-L272)
- [ArcaneManager.cs:39-56](file://Assets/Scripts/Battle/ArcaneManager.cs#L39-L56)
- [ArcaneManager.cs:119-165](file://Assets/Scripts/Battle/ArcaneManager.cs#L119-L165)

**章节来源**
- [arcane_config.json:1-80](file://Assets/Resources/Configs/arcane_config.json#L1-L80)
- [GameConfigs.cs:429-446](file://Assets/Scripts/Data/GameConfigs.cs#L429-L446)
- [ConfigManager.cs:77-122](file://Assets/Scripts/Core/ConfigManager.cs#L77-L122)
- [ArcaneManager.cs:198-256](file://Assets/Scripts/Battle/ArcaneManager.cs#L198-L256)
- [BattleManager.cs:419-453](file://Assets/Scripts/Battle/BattleManager.cs#L419-L453)

### 激活触发与效果实现
- 触发条件
  - CanCast：冷却归零且当前符能 ≥ 修饰后的 runeCost。
- 激活过程
  - TryCastArcane：扣减符能、设置冷却、创建 ActiveArcane 并加入列表。
  - Update：遍历 slots 更新冷却；遍历 activeArcanes 更新 tickTimer，周期到触发 TickArcane。
- 效果实现
  - TickArcane：按配置计算实际伤害（基于英雄基础攻击与配置倍率），范围命中敌人并执行 enemyEvents；同时对施法者执行 events。
  - 全屏AoE：radius < 0 时对全体敌人执行伤害与事件。
  - VFX：SpawnTickVfx 根据元素类型生成圆形扩散特效。

```mermaid
sequenceDiagram
participant AM as "ArcaneManager"
participant BM as "BattleManager"
participant E as "EventExecutor"
AM->>AM : "Update : 遍历activeArcanes"
AM->>AM : "tickTimer<=0? 触发TickArcane"
AM->>BM : "按半径查询敌人"
AM->>E : "执行enemyEvents(对目标)"
AM->>E : "执行events(对施法者)"
E-->>BM : "Apply伤害/效果"
```

**图表来源**
- [ArcaneManager.cs:167-196](file://Assets/Scripts/Battle/ArcaneManager.cs#L167-L196)
- [ArcaneManager.cs:198-256](file://Assets/Scripts/Battle/ArcaneManager.cs#L198-L256)
- [BattleManager.cs:381-395](file://Assets/Scripts/Battle/BattleManager.cs#L381-L395)
- [BattleManager.cs:419-453](file://Assets/Scripts/Battle/BattleManager.cs#L419-L453)
- [EventExecutor.cs:13-20](file://Assets/Scripts/Battle/EventExecutor.cs#L13-L20)

**章节来源**
- [ArcaneManager.cs:167-196](file://Assets/Scripts/Battle/ArcaneManager.cs#L167-L196)
- [ArcaneManager.cs:198-256](file://Assets/Scripts/Battle/ArcaneManager.cs#L198-L256)
- [BattleManager.cs:381-395](file://Assets/Scripts/Battle/BattleManager.cs#L381-L395)
- [BattleManager.cs:419-453](file://Assets/Scripts/Battle/BattleManager.cs#L419-L453)
- [EventExecutor.cs:13-20](file://Assets/Scripts/Battle/EventExecutor.cs#L13-L20)

### 符文组合系统与策略深度
- 当前实现
  - 单个符文独立配置，通过 events/enemyEvents/bulletEvents 实现效果组合。
  - 未见"多符文组合"专属配置或规则，组合效果由多个符文的事件叠加实现。
- 策略维度
  - 元素克制：不同 dmgType 的符文对不同敌人有差异化效果。
  - 冷却与能量：符能收集与冷却节奏影响连携与爆发。
  - 范围与全屏：半径与全屏AoE的选择决定清线与控场策略。
  - **新增** 自动绑定系统：简化了符文选择界面的开发，提升了代码可维护性。
  - **新增** 拖拽提示：dragHint 提供更直观的操作指导，提升用户体验。
- 扩展建议
  - 在 BuffSystem 或 ArcaneManager 增加"组合效果"判定与加成逻辑，以符文ID集合为键进行组合加成。

**章节来源**
- [ArcaneManager.cs:108-133](file://Assets/Scripts/Battle/ArcaneManager.cs#L108-L133)
- [BuffSystem.cs:285-303](file://Assets/Scripts/Battle/BuffSystem.cs#L285-L303)
- [bullet_event_config.json:1-37](file://Assets/Resources/Configs/bullet_event_config.json#L1-L37)

### 平衡性设计
- 能量消耗限制
  - runeCost：基础消耗；BuffSystem.GetArcaneCostModifier 可加成/减免。
- 冷却时间设置
  - cd：冷却时间，ArcaneSlotUI 显示剩余时间；ArcaneManager.Update 逐槽冷却。
- 效果强度调节
  - dmg 与 tickInterval：dmg 决定单次伤害，tickInterval 决定周期频率。
  - radius：范围AoE；radius < 0 为全屏AoE。
  - 元素类型 dmgType：影响伤害计算与视觉反馈。
- **新增** 自动绑定系统平衡
  - 通过统一的命名约定和自动绑定机制，减少了手动绑定错误，提升了开发效率。
- **新增** 拖拽提示平衡
  - dragHint 提供操作指导，减少误操作，提升游戏体验。

**章节来源**
- [ArcaneManager.cs:108-133](file://Assets/Scripts/Battle/ArcaneManager.cs#L108-L133)
- [ArcaneManager.cs:167-196](file://Assets/Scripts/Battle/ArcaneManager.cs#L167-L196)
- [ArcaneManager.cs:198-256](file://Assets/Scripts/Battle/ArcaneManager.cs#L198-L256)
- [ArcaneSlotUI.cs:63-78](file://Assets/Scripts/UI/Components/ArcaneSlotUI.cs#L63-L78)
- [GameConfigs.cs:429-446](file://Assets/Scripts/Data/GameConfigs.cs#L429-L446)

### 扩展指南
- 添加新符文类型
  - 在 arcane_config.json 中新增 ArcaneConfig 条目，设置 id/name/des/icon/dmg/dmgType/radius/tickInterval/cd/runeCost/runeType/events/enemyEvents/bulletEvents/**dragHint**。
  - 在 ConfigManager 中无需改动（自动加载）。
- 修改符文效果
  - 通过 events/enemyEvents/bulletEvents 引用事件ID，事件由 EventExecutor 执行。
  - 若需全屏AoE，将 radius 设为负值。
- 实现自定义组合规则
  - 方案一：在 BuffSystem 或 ArcaneManager 增加组合判定（如特定符文ID集合触发加成）。
  - 方案二：在 ArcaneManager 中维护"已激活符文集合"，在 TickArcane 前后统一处理组合效果。
- **新增** 添加拖拽提示
  - 在符文配置中添加 `dragHint` 字段，提供针对该符文的拖拽操作指导。
  - 确保 DragVisualManager 正确处理 dragHint 参数。
- **新增** 使用自动绑定系统
  - 新建窗口类继承 BaseWin，按照命名约定创建UI组件（txt_、btn_、node_等前缀）。
  - 字段名与组件名匹配即可自动绑定，无需手动赋值。
  - Button 和 Toggle 组件会自动绑定事件处理器。

**章节来源**
- [arcane_config.json:1-80](file://Assets/Resources/Configs/arcane_config.json#L1-L80)
- [ConfigManager.cs:77-122](file://Assets/Scripts/Core/ConfigManager.cs#L77-L122)
- [EventExecutor.cs:13-20](file://Assets/Scripts/Battle/EventExecutor.cs#L13-L20)
- [bullet_event_config.json:1-37](file://Assets/Resources/Configs/bullet_event_config.json#L1-L37)
- [BuffSystem.cs:285-303](file://Assets/Scripts/Battle/BuffSystem.cs#L285-L303)
- [ArcaneConfig.cs:27](file://Assets/Scripts/Data/Configs/ArcaneConfig.cs#L27)
- [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)

## 依赖关系分析
- ArcaneManager 依赖
  - ConfigManager：读取 ArcaneConfig。
  - BattleManager：范围查询、全屏AoE、敌人接口。
  - EventExecutor：执行事件链。
  - BuffSystem：获取符能消耗修饰。
- UI 依赖
  - ArcaneBarUI/ArcaneSlotUI：绑定 ArcaneManager，监听槽位状态与符能变化。
  - RuneBarUI：订阅 OnRunesChanged，实时刷新符能与能量。
  - ArcaneActiveIconUI：订阅 OnArcanePlaced，显示激活图标与剩余时间。
  - **新增** ArcaneSelectWin：继承 BaseWin，利用自动绑定系统管理符文选择界面。
  - **新增** BaseWin：提供自动UI绑定功能，支持多种命名约定。
  - **新增** DragVisualManager：管理拖拽过程中的视觉效果和提示。
  - **新增** DragHintUI：显示拖拽提示的UI组件。

```mermaid
graph LR
AM["ArcaneManager"] --> CM["ConfigManager"]
AM --> BM["BattleManager"]
AM --> EE["EventExecutor"]
AM --> BS["BuffSystem"]
AM --> AB["ArcaneBarUI"]
AM --> AS["ArcaneSlotUI"]
AM --> RB["RuneBarUI"]
AM --> AI["ArcaneActiveIconUI"]
ASEL["ArcaneSelectWin"] --> BASEWIN["BaseWin"]
ASEL --> DV["DragVisualManager"]
BASEWIN --> DV
DV --> DH["DragHintUI"]
ASEL --> AM
```

**图表来源**
- [ArcaneManager.cs:39-56](file://Assets/Scripts/Battle/ArcaneManager.cs#L39-L56)
- [ArcaneManager.cs:198-256](file://Assets/Scripts/Battle/ArcaneManager.cs#L198-L256)
- [ArcaneBarUI.cs:11-18](file://Assets/Scripts/UI/ArcaneBarUI.cs#L11-L18)
- [RuneBarUI.cs:22-27](file://Assets/Scripts/UI/RuneBarUI.cs#L22-L27)
- [ArcaneActiveIconUI.cs:14-19](file://Assets/Scripts/UI/ArcaneActiveIconUI.cs#L14-L19)
- [ArcaneSelectWin.cs:14](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L14)
- [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)
- [DragVisualManager.cs:21](file://Assets/Scripts/UI/Components/DragVisualManager.cs#L21)
- [DragHintUI.cs:10](file://Assets/Scripts/UI/Components/DragHintUI.cs#L10)

**章节来源**
- [ArcaneManager.cs:39-56](file://Assets/Scripts/Battle/ArcaneManager.cs#L39-L56)
- [ArcaneManager.cs:198-256](file://Assets/Scripts/Battle/ArcaneManager.cs#L198-L256)
- [ArcaneBarUI.cs:11-18](file://Assets/Scripts/UI/ArcaneBarUI.cs#L11-L18)
- [RuneBarUI.cs:22-27](file://Assets/Scripts/UI/RuneBarUI.cs#L22-L27)
- [ArcaneActiveIconUI.cs:14-19](file://Assets/Scripts/UI/ArcaneActiveIconUI.cs#L14-L19)
- [ArcaneSelectWin.cs:14](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L14)
- [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)
- [DragVisualManager.cs:21](file://Assets/Scripts/UI/Components/DragVisualManager.cs#L21)
- [DragHintUI.cs:10](file://Assets/Scripts/UI/Components/DragHintUI.cs#L10)

## 性能考量
- 时间复杂度
  - Update 每帧遍历 slots 与 activeArcanes，复杂度 O(S+A)，S 为槽位数，A 为激活符文数。
  - 范围查询 GetEnemiesInRadius 遍历存活敌人，复杂度 O(N)，N 为敌人数量。
  - **新增** ArcaneSelectWin 的选择状态更新：HashSet 操作复杂度 O(1)，UI 刷新复杂度 O(M)，M 为符文数量。
  - **新增** BaseWin 自动绑定系统：反射操作复杂度 O(F×C)，F 为字段数，C 为子对象数。
- 优化建议
  - 使用空间换时间：缓存敌人包围盒或四叉树以降低范围查询成本。
  - 合理控制激活符文数量：限制最大激活数，避免过多 tick 计算。
  - UI刷新去抖：RuneBarUI 已通过定时刷新，ArcaneSlotUI 可考虑节流。
  - **新增** 拖拽提示优化：DragHintUI 使用一次性创建和销毁模式，避免频繁UI创建开销。
  - **新增** 自动绑定优化：BaseWin.AutoBindUIComponents 只在初始化时执行一次，避免重复绑定。

## 故障排查指南
- 常见问题
  - 符文无法施放：检查 CanCast 返回值，确认冷却是否归零、符能是否足够、Buff 是否增加消耗。
  - 伤害异常：核对 ArcaneConfig.dmg 与 dmgType，确认 BattleManager 的全屏AoE路径与范围命中逻辑。
  - 事件未生效：检查 events/enemyEvents/bulletEvents 的事件ID是否存在，EventExecutor 是否正确执行。
  - **新增** 自动绑定失败：检查字段命名是否符合 BaseWin 的命名约定，确保子对象名称与字段名匹配。
  - **新增** 拖拽提示不显示：检查 ArcaneConfig.dragHint 是否正确配置，DragVisualManager 是否正确接收和显示提示。
  - **新增** 选择界面异常：检查 ArcaneSelectWin 的 UI 组件命名是否正确，selectedIds 集合是否正常更新。
- 调试要点
  - 在 ArcaneManager.TickArcane 中断点，观察实际伤害与事件链。
  - 在 EventExecutor.HandleGainEnergy 中断点，确认能量收集是否按预期触发。
  - 在 BuffSystem.GetArcaneCostModifier 中断点，确认符能消耗修饰是否生效。
  - **新增** 在 BaseWin.AutoBindUIComponents 中断点，确认自动绑定是否正确执行。
  - **新增** 在 DragVisualManager.BeginDrag 中断点，确认 dragHint 参数传递正确。
  - **新增** 在 ArcaneSelectWin.UpdateVisuals 中断点，确认选择状态更新逻辑。

**章节来源**
- [ArcaneManager.cs:119-165](file://Assets/Scripts/Battle/ArcaneManager.cs#L119-L165)
- [ArcaneManager.cs:198-256](file://Assets/Scripts/Battle/ArcaneManager.cs#L198-L256)
- [EventExecutor.cs:124-143](file://Assets/Scripts/Battle/EventExecutor.cs#L124-L143)
- [BuffSystem.cs:285-303](file://Assets/Scripts/Battle/BuffSystem.cs#L285-L303)
- [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)
- [DragVisualManager.cs:29](file://Assets/Scripts/UI/Components/DragVisualManager.cs#L29)
- [ArcaneSelectWin.cs:137-148](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L137-148)

## 结论
GeometryTD 的奥术系统以配置驱动为核心，通过 ArcaneManager 统一管理符文槽位、能量与冷却，配合 ConfigManager、EventExecutor 与 BuffSystem 实现灵活的效果与修饰。UI 层提供直观的交互与反馈。当前版本侧重单符文效果叠加，组合策略可通过扩展 BuffSystem 或 ArcaneManager 的组合判定进一步深化。

**更新** 本次更新显著增强了系统的可维护性和开发效率，ArcaneSelectWin.cs 的完全重构引入了全新的自动绑定系统，大幅简化了UI组件的初始化过程。BaseWin 的 AutoBindUIComponents 功能支持多种命名约定，包括 txt_、btn_、toggle_、node_ 等前缀，为后续的UI开发提供了标准化的解决方案。这些改进使得系统更加易用和专业，为后续的功能扩展奠定了良好的基础。

建议在后续迭代中继续完善自动绑定系统，考虑添加更多命名约定支持，并优化符文选择界面的视觉设计，以进一步提升整体的游戏体验。

## 附录
- 配置示例路径
  - 符文配置：[arcane_config.json:1-80](file://Assets/Resources/Configs/arcane_config.json#L1-L80)
  - 子弹事件配置：[bullet_event_config.json:1-37](file://Assets/Resources/Configs/bullet_event_config.json#L1-L37)
- 数据模型参考
  - 符文配置结构：[GameConfigs.cs:429-446](file://Assets/Scripts/Data/GameConfigs.cs#L429-L446)
  - **新增** 拖拽提示配置： [ArcaneConfig.cs:27](file://Assets/Scripts/Data/Configs/ArcaneConfig.cs#L27)
  - 事件类型与参数： [GameConfigs.cs:122-170](file://Assets/Scripts/Data/GameConfigs.cs#L122-L170)
  - 事件执行入口： [EventExecutor.cs:13-63](file://Assets/Scripts/Battle/EventExecutor.cs#L13-L63)
- **新增** UI组件参考
  - 符文选择界面： [ArcaneSelectWin.cs:14](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L14)
  - 自动绑定系统： [BaseWin.cs:25-96](file://Assets/Scripts/UI/Windows/BaseWin.cs#L25-L96)
  - 拖拽视觉管理： [DragVisualManager.cs:21](file://Assets/Scripts/UI/Components/DragVisualManager.cs#L21)
  - 拖拽提示UI： [DragHintUI.cs:10](file://Assets/Scripts/UI/Components/DragHintUI.cs#L10)
  - 符文选择界面预制体： [ArcaneSelectWin.prefab](file://Assets/Resources/UI/Windows/ArcaneSelectWin.prefab)