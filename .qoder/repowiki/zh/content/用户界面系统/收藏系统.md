...
<cite>
**本文档引用的文件**
- [StoryCollectionWin.cs](file://Assets/Scripts/UI/Windows/StoryCollectionWin.cs)
- [CollectionBagWin.cs](file://Assets/Scripts/UI/Windows/CollectionBagWin.cs)
...
## 更新摘要
**变更内容**
- 收藏系统UI组件现代化更新，采用标准化命名约定
- CollectionDetailWin.cs和CollectionBagWin.cs进行了大量UI结构重构
- 新增CollectionDetailWinParam和CollectionBagWinParam参数类
- 优化了UI组件的命名规范和结构组织
- 增强了收藏效果的展示和交互体验
...
**更新** 系统经过现代化重构，采用了更标准化的UI组件命名约定和结构组织，提升了代码的可维护性和一致性。
...
E[CollectionDetailWinParam<br/>详情窗口参数类]
F[CollectionBagWinParam<br/>背包窗口参数类]
end
subgraph "核心逻辑层"
G[StoryManager.cs<br/>故事管理器]
H[StoryRuntime.cs<br/>运行时状态]
I[StorySaveManager.cs<br/>存档管理器]
end
subgraph "配置层"
J[ConfigManager.cs<br/>配置管理器]
K[Cfg.cs<br/>配置访问器]
L[StoryCollectionConfig.cs<br/>故事集配置]
M[PassiveEffectConfig.cs<br/>被动效果配置]
end
subgraph "数据层"
N[story_collection_config.json<br/>故事集配置数据]
O[passive_effect_config.json<br/>被动效果配置数据]
end
A --> G
B --> G
C --> G
...
**图表来源**
- [StoryCollectionWin.cs:1-612](file://Assets/Scripts/UI/Windows/StoryCollectionWin.cs#L1-L612)
- [CollectionBagWin.cs:1-200](file://Assets/Scripts/UI/Windows/CollectionBagWin.cs#L1-L200)
- [CollectionDetailWin.cs:1-207](file://Assets/Scripts/UI/Windows/CollectionDetailWin.cs#L1-L207)
- [StoryManager.cs:1-574](file://Assets/Scripts/Core/StoryManager.cs#L1-L574)
- [StoryRuntime.cs:1-380](file://Assets/Scripts/Data/StoryRuntime.cs#L1-L380)
...
UI4[CollectionDetailWinParam<br/>详情参数]
UI5[CollectionBagWinParam<br/>背包参数]
end
subgraph "业务逻辑层"
...
**图表来源**
- [StoryCollectionWin.cs:276-308](file://Assets/Scripts/UI/Windows/StoryCollectionWin.cs#L276-L308)
- [StoryManager.cs:486-511](file://Assets/Scripts/Core/StoryManager.cs#L486-L511)
- [ConfigManager.cs:115-119](file://Assets/Scripts/Core/ConfigManager.cs#L115-L119)
...
+Text txt_title
+Transform node_content
+Button btn_close
+Text txt_count
-GameObject[] itemObjects
+CollectionBagWinParam data
+Init()
+Show()
+OnClose()
-RefreshItems()
-ClearItems()
-CreateCollectionItem(config, index)
-OnItemClicked(config)
}
class CollectionBagWinParam {
}
class StoryManager {
...
CollectionBagWin --> CollectionBagWinParam : 使用
CollectionBagWin --> StoryManager : 调用
StoryManager --> PassiveEffectConfig : 返回
```

**图表来源**
- [CollectionBagWin.cs:15-170](file://Assets/Scripts/UI/Windows/CollectionBagWin.cs#L15-L170)
...
**图表来源**
- [CollectionDetailWin.cs:45-132](file://Assets/Scripts/UI/Windows/CollectionDetailWin.cs#L45-L132)
- [CollectionDetailWin.cs:156-202](file://Assets/Scripts/UI/Windows/CollectionDetailWin.cs#L156-L202)
...
**图表来源**
- [StoryRuntime.cs:11-296](file://Assets/Scripts/Data/StoryRuntime.cs#L11-L296)
- [StoryRuntime.cs:302-310](file://Assets/Scripts/Data/StoryRuntime.cs#L302-L310)
...
**更新** 系统经过现代化重构，采用了更标准化的命名约定和UI结构，提升了代码质量和开发效率。新的参数类设计和组件化架构使得系统更加模块化和易于维护。