...
## 更新摘要
**变更内容**
- 重大更新：BaseWin类从传统的Init()和Show()方法重构为新的参数化生命周期管理
- 引入了完整的参数化系统：Init(object param)、load()、start()、closeWin()等方法
- 新增了参数类设计模式：LevelSelectWinParam、ChoiceWinParam、DialogueWinParam等
- 更新了窗口打开方式：GameHelper.OpenWin<T>(param)支持参数传递
- 优化了窗口重用机制：ResetOpen(object param)支持参数重置
- 更新了项目结构图和文件路径引用
...
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [参数化生命周期管理](#参数化生命周期管理)
7. [参数类设计模式](#参数类设计模式)
8. [依赖关系分析](#依赖关系分析)
9. [性能与最佳实践](#性能与最佳实践)
10. [故障排查指南](#故障排查指南)
11. [结论](#结论)
12. [附录：窗口扩展指南](#附录窗口扩展指南)
...
**更新** 本次重大更新反映了BaseWin类从传统生命周期管理向参数化生命周期管理的重构，引入了完整的参数传递系统，使窗口能够接收和处理运行时参数，大大增强了窗口的灵活性和可重用性。

## 项目结构
窗口系统位于UI层，核心由以下模块构成：
- 基类层：BaseWin 提供统一的参数化窗口生命周期与显示控制接口
...
- [LevelSelectWin.cs:15-166](file://Assets/Scripts/UI/Windows/LevelSelectWin.cs#L15-L166)
...
## 核心组件
- BaseWin：抽象基类，定义窗口的参数化生命周期接口：Init(object param)/load()/start()/Hide/OnClose/closeWin()。派生类可覆盖各阶段方法以实现自定义行为。新增WinType和WinPriority属性用于窗口分类和层级控制。
...
**更新** 新增了完整的参数化生命周期管理机制，支持窗口接收运行时参数并进行相应的初始化和显示处理。
...
**更新** 新增了完整的参数化生命周期管理，支持窗口接收和处理运行时参数。
...
**更新** LevelSelectWin现在使用参数化设计，通过LevelSelectWinParam参数类接收关卡ID，实现了更灵活的窗口初始化。
...
- [LevelSelectWin.cs:144-163](file://Assets/Scripts/UI/Windows/LevelSelectWin.cs#L144-L163)

**章节来源**
- [LevelSelectWin.cs:1-166](file://Assets/Scripts/UI/Windows/LevelSelectWin.cs#L1-L166)
...
**更新** ArcaneSelectWin现在遵循参数化生命周期模式，虽然当前参数类为空，但为未来的功能扩展预留了参数传递机制。
...
**章节来源**
- [ArcaneSelectWin.cs:1-163](file://Assets/Scripts/UI/Windows/ArcaneSelectWin.cs#L1-L163)
- [ConfigManager.cs:77-122](file://Assets/Scripts/Core/ConfigManager.cs#L77-L122)
...
**更新** ChoiceWin现在使用参数化设计，通过ChoiceWinParam参数类接收ChoiceGroupConfig配置和Action回调函数，实现了更灵活的窗口初始化。
...
**章节来源**
- [ChoiceWin.cs:1-303](file://Assets/Scripts/UI/Windows/ChoiceWin.cs#L1-L303)
...
**更新** DialogueWin现在使用参数化设计，通过DialogueWinParam参数类接收对话ID和Action回调函数，实现了更灵活的对话窗口初始化。
...
**章节来源**
- [BaseWin.cs:98-173](file://Assets/Scripts/UI/Windows/BaseWin.cs#L98-L173)
- [WinManager.cs:63-114](file://Assets/Scripts/UI/Managers/WinManager.cs#L63-L114)
- [GameHelper.cs:78-82](file://Assets/Scripts/Core/GameHelper.cs#L78-L82)
...
**更新** 性能优化方面，参数化生命周期管理显著提升了加载和渲染效率，特别是LevelSelectWin等窗口的响应速度得到了明显改善。
...
**更新** BaseWin类的参数化生命周期管理重构展示了系统向更高效、更灵活的方向发展，通过参数传递机制和清晰的生命周期管理，显著提升了窗口的可重用性和维护性。新的层级管理机制和简化的UI结构为未来的窗口开发奠定了良好的基础。
...
**更新** 扩展指南增加了关于参数化生命周期管理和参数类设计的最佳实践建议。
