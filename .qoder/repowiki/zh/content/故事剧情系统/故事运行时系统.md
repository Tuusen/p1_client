...
**变更内容**
- 新增了失败重试功能的跨游戏会话支持机制
- StoryRuntime数据结构新增failedAtNodeId字段，用于跟踪玩家失败时的故事节点
- StoryManager中新增failedAtNodeId私有字段和相关处理逻辑
- 实现了从失败结局节点重试的功能，支持重启游戏后的状态恢复
...
**更新** 本版本新增了失败重试功能的跨游戏会话支持，通过failedAtNodeId字段实现了从失败结局节点重试的能力。
...
## 核心组件
- StoryRuntime：承载一次冒险的全部运行时状态，支持序列化/反序列化，提供选择记录、访问历史、金币、藏品效果、节点移动与下一节点解析等能力。**新增**：failedAtNodeId字段用于跟踪失败时的节点ID。
- StorySaveManager：负责运行时中途存档（StorySaveData）与永久进度（StoryProgressData）的保存、读取、删除与缓存。
- StoryManager：单例，协调节点推进、事件触发、场景切换、金币与藏品系统，并通过事件通知 UI 更新。**新增**：failedAtNodeId私有字段和失败重试逻辑。
...
- 关键字段与职责
  ...
  - **新增**：failedAtNodeId：战斗失败前的节点ID（用于重启游戏后的重试功能）
- 辅助方法
...
### 失败重试功能：failedAtNodeId 字段与跨会话支持
**更新** 这是本次应用变更的核心改进，新增了失败重试功能的跨游戏会话支持。

- 功能概述
  - failedAtNodeId字段用于记录玩家在战斗失败前所在的节点ID
  - 支持重启游戏后从失败节点重新开始挑战
  - 通过StoryManager的failedAtNodeId私有字段和Runtime.failedAtNodeId实现双重备份
- 核心实现机制
  - 战斗失败时：HandleBattleFailed方法捕获当前节点ID并保存到failedAtNodeId
  - 节点配置：StoryNodeConfig中failNodeId字段指向失败结局节点
  - 重试逻辑：RetryFromFailure方法优先从Runtime.failedAtNodeId恢复，支持跨会话重试
  - 状态清理：重试完成后清除失败状态，防止重复重试
- 状态管理流程

```mermaid
flowchart TD
Start(["战斗失败"]) --> Capture["Capture failedAtNodeId"]
Capture --> SaveRuntime["保存到Runtime.failedAtNodeId"]
SaveRuntime --> JumpFail["跳转到失败节点"]
JumpFail --> WaitRetry["等待重试请求"]
WaitRetry --> CheckRuntime{"检查Runtime.failedAtNodeId"}
CheckRuntime --> |存在| UseRuntime["使用Runtime.failedAtNodeId"]
CheckRuntime --> |不存在| UseLocal["使用本地failedAtNodeId"]
UseRuntime --> PrepareRetry["准备重试状态"]
UseLocal --> PrepareRetry
PrepareRetry --> ClearVisited["移除失败节点访问记录"]
ClearVisited --> ClearChoices["清除失败节点选择记录"]
ClearChoices --> SetCurrent["设置当前节点为失败前节点"]
SetCurrent --> SaveFinal["保存最终状态"]
SaveFinal --> Success["重试成功"]
Success --> End(["结束"])
```

**图表来源**
- [StoryManager.cs:196-254](file://Assets/Scripts/Core/StoryManager.cs#L196-L254)
- [StoryNodeConfig.cs:38](file://Assets/Scripts/Data/Configs/StoryNodeConfig.cs#L38)
...
- 空引用处理
  ...
  - **新增**：failedAtNodeId在RetryFromFailure中进行有效性检查
- 边界条件保护
  ...
  - **新增**：failedAtNodeId重试逻辑的边界检查，防止无效重试
- 完整性保障
  - 使用 JsonUtility 序列化/反序列化，保证跨平台一致性
  - 运行时存档在关键节点变更后立即保存，降低丢失风险
  - 永久进度独立存储，不受单次冒险影响
  - **新增**：failedAtNodeId双重备份机制，确保跨会话状态恢复
...
- [StoryManager.cs:226-254](file://Assets/Scripts/Core/StoryManager.cs#L226-L254)
...
- 新增运行时字段
  ...
  - **新增**：failedAtNodeId字段的持久化已在现有框架中自动支持
- 修改数据结构
...
- 自定义状态管理功能
  ...
  - **新增**：失败重试功能的扩展点，可通过failNodeId配置实现自定义失败处理
- **新增**：无限制堆叠配置
...
- **新增**：失败重试功能
  - 战斗失败处理：[StoryManager.cs:196-220](file://Assets/Scripts/Core/StoryManager.cs#L196-L220)
  - 从失败节点重试：[StoryManager.cs:226-254](file://Assets/Scripts/Core/StoryManager.cs#L226-L254)
  - 节点配置中的失败节点：[StoryNodeConfig.cs:38](file://Assets/Scripts/Data/Configs/StoryNodeConfig.cs#L38)
- **新增**：无限制堆叠使用场景
...
- **新增**：StoryNodeConfig 依赖 StoryNodeConfig.NextNodesItem 进行节点连接
...
- 数据结构
  ...
  - **新增**：failedAtNodeId字段为简单整型，内存占用极小，性能影响可忽略
...
- **新增**：失败重试功能异常
  - 现象：RetryFromFailure 返回 false 或重试无效
  - 排查：确认StoryNodeConfig中failNodeId配置是否正确；检查failedAtNodeId是否被正确保存；验证Runtime.failedAtNodeId是否在重启后仍有效
  - 参考：[StoryManager.cs:196-220](file://Assets/Scripts/Core/StoryManager.cs#L196-L220)
  - 参考：[StoryManager.cs:226-254](file://Assets/Scripts/Core/StoryManager.cs#L226-L254)
- **新增**：无限制堆叠异常
...
**更新** 本次应用变更显著增强了故事系统的用户体验，通过新增failedAtNodeId字段和失败重试功能，实现了跨游戏会话的失败重试支持。同时，无限制堆叠机制的引入为游戏平衡性和玩家体验提供了更精细的控制。这些改进通过合理的边界保护与清晰的扩展路径，确保了系统的稳定性和可维护性。

通过持续的优化和扩展，开发者可以在不破坏现有流程的前提下，安全地添加新状态字段、自定义逻辑以及支持无限制堆叠的特殊藏品，进一步丰富游戏内容。
...
- 配置文件说明
  - 故事节点配置：描述节点类型、默认走向、失败节点、分支与条件
    ...
    - 节点配置类定义：[StoryNodeConfig.cs:10-43](file://Assets/Scripts/Data/Configs/StoryNodeConfig.cs#L10-L43)
...
- **新增**：失败重试配置示例
  - 在StoryNodeConfig中设置failNodeId指向失败结局节点
  - 系统自动捕获失败前的节点ID并保存到failedAtNodeId
  - 支持重启游戏后的跨会话重试
- **新增**：无限制堆叠配置示例
