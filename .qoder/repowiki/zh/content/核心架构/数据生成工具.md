# 数据生成工具

<cite>
**本文档引用的文件**
- [config_gen.py](file://Tools/config_gen.py)
- [ConfigManager.cs](file://Assets/Scripts/Core/ConfigManager.cs)
- [Cfg.cs](file://Assets/Scripts/Core/Cfg.cs)
- [GameTypes.cs](file://Assets/Scripts/Data/GameTypes.cs)
- [hero_config.json](file://Assets/Resources/Configs/hero_config.json)
- [skill_config.json](file://Assets/Resources/Configs/skill_config.json)
- [level_config.json](file://Assets/Resources/Configs/level_config.json)
- [global_config.json](file://Assets/Resources/Configs/global_config.json)
</cite>

## 更新摘要
**所做更改**
- 移除了所有关于Python配置迁移工具的内容（migrate_json_to_excel.py、migrate_separator.py、migrate_struct_types.py）
- 更新了项目结构图以反映单一工具链
- 删除了迁移器相关的工作流程描述
- 更新了架构概览以体现简化的工具链
- 强调了配置生成流程的内化和自动化集成

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

数据生成工具是Unity游戏项目GeometryTD中用于管理配置数据的核心系统。该系统提供了从Excel表格到JSON配置文件以及C#代码的完整转换流程，支持复杂的数据类型、结构体数组和元数据管理。

**更新** 该工具现已内化到Unity项目中，配置生成流程已完全集成到项目结构中，移除了外部Python工具依赖，实现了更简洁高效的工具链。

## 项目结构

项目采用模块化设计，将数据生成工具与游戏逻辑分离，并已完全内化到Unity项目中：

```mermaid
graph TB
subgraph "内化工具层"
CG[config_gen.py<br/>内化配置生成器]
END[自动执行<br/>项目构建流程]
end
subgraph "配置源"
XLSX[Excel配置文件<br/>Configs/目录]
end
subgraph "输出层"
JSON[JSON配置文件<br/>Assets/Resources/Configs]
CS[Unity C#配置类<br/>Assets/Scripts/Data/Configs]
RES[Unity资源<br/>Resources系统]
end
subgraph "游戏核心"
CM[ConfigManager.cs<br/>配置管理器]
CFG[Cfg.cs<br/>快捷访问器]
GT[GameTypes.cs<br/>通用类型定义]
end
CG --> XLSX
CG --> JSON
CG --> CS
CG --> RES
END --> CG
CM --> RES
CFG --> CM
GT --> CS
```

**图表来源**
- [config_gen.py:587-688](file://Tools/config_gen.py#L587-L688)

**章节来源**
- [config_gen.py:1-688](file://Tools/config_gen.py#L1-L688)

## 核心组件

### 内化配置生成器 (config_gen.py)

配置生成器是整个数据生成系统的核心，负责将Excel配置文件转换为JSON格式和对应的C#代码，并已完全内化到Unity项目中。

#### 类型系统
系统实现了完整的类型系统，支持基本类型、数组类型和结构体数组类型：

```mermaid
classDiagram
class TypeInfo {
<<abstract>>
}
class PrimitiveType {
+string name
+csharp_type
}
class ArrayType {
+TypeInfo element_type
+csharp_type
}
class StructArrayType {
+string struct_name
+list fields
+bool is_anonymous
+csharp_type
}
TypeInfo <|-- PrimitiveType
TypeInfo <|-- ArrayType
TypeInfo <|-- StructArrayType
```

**图表来源**
- [config_gen.py:22-93](file://Tools/config_gen.py#L22-L93)

#### 数据转换机制
系统支持多种数据类型的转换，包括原始类型、数组类型和结构体数组类型：

- **原始类型转换**：整数、浮点数、字符串、布尔值
- **数组类型转换**：使用'|'分隔符的数组
- **结构体数组转换**：使用'~'分隔符的结构体字段

**章节来源**
- [config_gen.py:100-184](file://Tools/config_gen.py#L100-L184)
- [config_gen.py:191-237](file://Tools/config_gen.py#L191-L237)

## 架构概览

数据生成系统的整体架构采用分层设计，现已完全内化到Unity项目中：

```mermaid
graph TB
subgraph "输入层"
EXCEL[Excel文件<br/>Configs/目录]
AUTOMATION[Unity自动化<br/>项目构建流程]
end
subgraph "处理层"
PARSER[Excel解析器]
CONVERTER[数据转换器]
VALIDATOR[数据验证器]
SHARED_TYPES[共享类型注册]
end
subgraph "输出层"
JSON_OUT[JSON文件<br/>Assets/Resources/Configs]
CS_OUT[C#代码<br/>Assets/Scripts/Data/Configs]
RESOURCES[Unity资源<br/>Resources系统]
end
subgraph "游戏集成"
CONFIG_MANAGER[ConfigManager]
CFG_ACCESSOR[Cfg访问器]
GAME_TYPES[GameTypes]
end
EXCEL --> PARSER
PARSER --> SHARED_TYPES
SHARED_TYPES --> CONVERTER
CONVERTER --> VALIDATOR
VALIDATOR --> JSON_OUT
VALIDATOR --> CS_OUT
JSON_OUT --> RESOURCES
CS_OUT --> GAME_TYPES
RESOURCES --> CONFIG_MANAGER
GAME_TYPES --> CONFIG_MANAGER
CONFIG_MANAGER --> CFG_ACCESSOR
AUTOMATION --> PARSER
```

**图表来源**
- [config_gen.py:587-688](file://Tools/config_gen.py#L587-L688)
- [ConfigManager.cs:11-306](file://Assets/Scripts/Core/ConfigManager.cs#L11-L306)

## 详细组件分析

### 配置管理系统

#### ConfigManager组件
ConfigManager是游戏中的配置管理核心，负责加载和管理所有配置数据：

```mermaid
classDiagram
class ConfigManager {
+static ConfigManager Instance
+ConfigTable~ArcaneConfig,ArcaneMeta~ arcaneTable
+ConfigTable~AttributeConfig~ attributeTable
+ConfigTable~BuffConfig~ buffTable
+LoadAllConfigs() void
+LoadConfig~T~(string path) T
+GetSkillConfigByPool(int poolId, int level) SkillConfig
+GetBossConfig() MonsterConfig
+GetNodesByCollection(int collectionId) StoryNodeConfig[]
}
class ConfigTable {
<<generic>>
+Init(T[], Func~T,int~) void
+Init(T[], TMeta, Func~T,int~) void
+Get(int id) T
+All T[]
}
class ConfigMeta {
<<generic>>
+Init(TMeta) void
+Get() TMeta
}
ConfigManager --> ConfigTable : "管理"
ConfigManager --> ConfigMeta : "管理"
```

**图表来源**
- [ConfigManager.cs:11-306](file://Assets/Scripts/Core/ConfigManager.cs#L11-L306)

#### Cfg快捷访问器
Cfg类提供了静态访问接口，简化了配置数据的获取：

```mermaid
sequenceDiagram
participant Game as 游戏代码
participant Cfg as Cfg访问器
participant Manager as ConfigManager
participant Table as ConfigTable
Game->>Cfg : Cfg.Hero.Get(id)
Cfg->>Manager : 获取实例
Manager->>Table : 返回HeroTable
Table->>Table : 查找配置项
Table-->>Game : 返回HeroConfig
```

**图表来源**
- [Cfg.cs:7-35](file://Assets/Scripts/Core/Cfg.cs#L7-L35)

**章节来源**
- [ConfigManager.cs:1-306](file://Assets/Scripts/Core/ConfigManager.cs#L1-L306)
- [Cfg.cs:1-35](file://Assets/Scripts/Core/Cfg.cs#L1-L35)

### 数据类型系统

#### 通用类型定义
GameTypes.cs定义了跨表共享的配置结构体和运行时数据：

```mermaid
classDiagram
class AttrEntry {
+int id
+int value
}
class BulletEventData {
+int pierceCount
+int explosionDmgRate
+int explosionRadius
+bool homing
+int scatterCount
+int scatterAngle
+int bounceCount
+Clone() BulletEventData
}
class SkillUseInfo {
+SkillUseResult result
+float cooldownRemaining
}
class SkillCategory {
<<enumeration>>
Self
Projectile
Aoe
Shield
Summon
}
class SkillUseResult {
<<enumeration>>
Success
LevelTooLow
OnCooldown
InvalidSlot
}
```

**图表来源**
- [GameTypes.cs:8-83](file://Assets/Scripts/Data/GameTypes.cs#L8-L83)

**章节来源**
- [GameTypes.cs:1-83](file://Assets/Scripts/Data/GameTypes.cs#L1-L83)

### 配置数据示例

#### 英雄配置示例
英雄配置展示了复杂的嵌套结构和数组类型：

```mermaid
erDiagram
HERO_CONFIG {
int id PK
string name
string description
int role
int[] attack_skill_ids
float skill_xp_interval
int skill_xp_min
int skill_xp_max
}
ATTR_ENTRY {
int id PK
int value
}
BUFF_ENTRY {
int id PK
int value
}
HERO_CONFIG ||--o{ ATTR_ENTRY : "包含"
HERO_CONFIG ||--o{ BUFF_ENTRY : "包含"
```

**图表来源**
- [hero_config.json:1-97](file://Assets/Resources/Configs/hero_config.json#L1-L97)

#### 关卡配置示例
关卡配置展示了复杂的数据结构和嵌套数组：

```mermaid
erDiagram
LEVEL_CONFIG {
int id PK
string name
string des
string bg
int[] conditions
int hard
float spawn_interval
int coinNormalKill
int coinEliteKill
int coinBossKill
float coinSelfDestructRate
}
MONSTER_ENTRY {
int id PK
int generate
}
SUPER_MONSTER_ENTRY {
int id PK
int num
int generate
}
BOSS_ENTRY {
int id PK
int num
}
LEVEL_CONFIG ||--o{ MONSTER_ENTRY : "包含"
LEVEL_CONFIG ||--o{ SUPER_MONSTER_ENTRY : "包含"
LEVEL_CONFIG ||--o{ BOSS_ENTRY : "包含"
```

**图表来源**
- [level_config.json:1-141](file://Assets/Resources/Configs/level_config.json#L1-L141)

**章节来源**
- [hero_config.json:1-97](file://Assets/Resources/Configs/hero_config.json#L1-L97)
- [level_config.json:1-141](file://Assets/Resources/Configs/level_config.json#L1-L141)

## 依赖关系分析

### 内部依赖关系
系统现已完全内化到Unity项目中，对外部依赖大幅减少：

```mermaid
graph TB
subgraph "内部依赖"
OPENPYXL[openpyxl==2.6.4<br/>仅用于配置生成]
UNITY[Unity引擎]
PYTHON[Python 2.7+<br/>仅用于配置生成]
END[Unity自动化<br/>项目构建流程]
end
subgraph "内部组件"
CONFIG_GEN[config_gen.py]
ASSETS[Unity资源系统]
SCRIPTS[Unity脚本系统]
end
OPENPYXL --> CONFIG_GEN
PYTHON --> CONFIG_GEN
UNITY --> CONFIG_GEN
CONFIG_GEN --> ASSETS
CONFIG_GEN --> SCRIPTS
END --> CONFIG_GEN
```

**图表来源**
- [config_gen.py:16](file://Tools/config_gen.py#L16)

### 内部依赖关系
组件间的依赖关系体现了清晰的分层架构：

```mermaid
graph TD
subgraph "内化工具层"
CONFIG_GEN[config_gen.py]
AUTOMATION[Unity自动化]
end
subgraph "配置层"
EXCEL_FILES[Excel配置文件<br/>Configs/目录]
JSON_FILES[JSON配置文件<br/>Assets/Resources/Configs]
CS_FILES[C#配置文件<br/>Assets/Scripts/Data/Configs]
end
subgraph "游戏层"
CONFIG_MANAGER[ConfigManager.cs]
CFG_ACCESSOR[Cfg.cs]
GAME_TYPES[GameTypes.cs]
end
CONFIG_GEN --> EXCEL_FILES
CONFIG_GEN --> JSON_FILES
CONFIG_GEN --> CS_FILES
JSON_FILES --> CONFIG_MANAGER
CS_FILES --> GAME_TYPES
CONFIG_MANAGER --> CFG_ACCESSOR
AUTOMATION --> CONFIG_GEN
```

**图表来源**
- [config_gen.py:587-688](file://Tools/config_gen.py#L587-L688)

**章节来源**
- [config_gen.py:16](file://Tools/config_gen.py#L16)

## 性能考虑

### 数据加载优化
配置管理系统采用了多种性能优化策略：

1. **延迟加载**：配置数据在首次访问时才进行解析
2. **缓存机制**：预加载常用配置到内存缓存
3. **增量更新**：支持部分配置的动态更新
4. **内存管理**：及时释放不再使用的配置数据

### 工具执行效率
数据生成工具在处理大量配置文件时采用了以下优化：

1. **批量处理**：一次处理多个Excel文件
2. **预扫描机制**：先扫描所有文件以收集结构定义
3. **流式处理**：避免一次性加载整个工作簿
4. **错误恢复**：单个文件错误不影响整体处理

## 故障排除指南

### 常见问题及解决方案

#### Excel文件格式问题
- **问题**：Excel文件无法正确解析
- **原因**：文件格式不兼容或损坏
- **解决**：使用Microsoft Excel重新保存文件

#### 类型转换错误
- **问题**：配置数据转换失败
- **原因**：数据类型不匹配或格式错误
- **解决**：检查Excel中的类型声明和数据格式

#### JSON生成问题
- **问题**：生成的JSON文件格式错误
- **原因**：特殊字符处理不当
- **解决**：检查分隔符使用和转义字符

#### 运行时配置加载失败
- **问题**：游戏启动时配置加载失败
- **原因**：资源配置路径错误或文件缺失
- **解决**：验证Resources目录结构和文件名

**章节来源**
- [config_gen.py:550-566](file://Tools/config_gen.py#L550-L566)
- [ConfigManager.cs:179-194](file://Assets/Scripts/Core/ConfigManager.cs#L179-L194)

## 结论

数据生成工具为GeometryTD项目提供了一个完整、灵活且高效的配置管理解决方案。通过内化的config_gen.py配置生成器，实现了从Excel到JSON再到C#代码的完整转换流程，支持复杂的数据类型和结构体数组。

**更新** 该系统经过内化后，主要优势包括：

1. **内化集成**：配置生成流程已完全集成到Unity项目中
2. **自动化执行**：通过Unity自动化机制实现项目构建流程
3. **简化维护**：单一工具链减少了维护复杂度
4. **灵活性**：支持多种数据类型和复杂结构
5. **可维护性**：提供完整的数据转换工具
6. **性能**：优化的加载和缓存机制
7. **扩展性**：模块化的架构设计便于功能扩展

通过标准化的配置管理流程，开发者可以更专注于游戏内容的创作，而无需担心底层数据管理的复杂性。该系统为类似的游戏项目提供了一个优秀的参考实现。