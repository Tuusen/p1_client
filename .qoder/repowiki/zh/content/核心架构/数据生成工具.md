...
- 新增了多工作表处理增强章节，详细说明parse_excel函数的改进
...
parse_excel函数现在返回多个结果，每个工作表对应一个配置文件。该函数经过重大增强，增加了21行代码来支持多工作表处理：
...
**更新** parse_excel函数的关键增强包括：

1. **多工作表遍历**：使用`for ws_name in wb.sheetnames:`循环处理所有工作表
2. **智能工作表命名**：根据工作表名称智能确定配置文件名
3. **元数据工作表过滤**：自动跳过以'_meta'结尾的工作表
4. **工作表有效性检查**：只处理包含实际数据的工作表
...
- [Cfg.cs:1-35](file://Assets/Scripts/Core/Cfg.cs#L1-L35)
- [GameTypes.cs:1-83](file://Assets/Scripts/Data/GameTypes.cs#L1-L83)
- [HeroConfig.cs:1-31](file://Assets/Scripts/Data/Configs/HeroConfig.cs#L1-L31)